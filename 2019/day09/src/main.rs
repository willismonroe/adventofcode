...
impl IntCodeComputer {
    ...
    fn _fetch_parameter(&mut self, mode: i64) -> Parameter {
        ...
        match mode {
            ...
            _ => panic!("Unknown parameter word: {}", mode),
        }
    }
    ...
    fn fetch_param3(&mut self, mut opcode: i64) -> (Parameter, Parameter, Parameter) {
        ...
        opcode /= 100;
        let p3 = self._fetch_parameter(opcode);
        (p1, p2, p3)
    }
    ...
    fn unwrap_value(&self, param: Parameter) -> i64 {
        match param {
            ...
            Parameter::Relative(tar) => self.memory[(tar + self.relative_base) as usize],
        }
    }
    ...
    fn store_val(&mut self, param: Parameter, val: i64) {
        match param {
            ...
            Parameter::Relative(tar) => {
                self.store_value_at_pos(tar + self.relative_base, val);
            }
            _ => panic!("Can't store to parameter in immediate mode."),
        }
    }

    fn emit_ouput(&mut self, param: Parameter) {
    ...
    fn fetch_instruction(&mut self) -> Instruction {
        ...
        match inst {
            ...
            9 => Instruction::AdjustBase(self.fetch_param1(opcode)),
            99 => Instruction::Halt,
    ...
    fn tick(&mut self) -> Signal {
        ...
        match inst {
            ...
            Instruction::AdjustBase(param) => {
                self.relative_base += self.unwrap_value(param);
                return Signal::None;
            }
...
fn main() {
    ...
    println!("Part 1: {:?}", c.get_output().unwrap());
    let mut c = get_computer(&input, vec![2]);
    c.run_till_signal(Signal::ProducedOutput);
    println!("Part 2: {:?}", c.get_output().unwrap());
}
